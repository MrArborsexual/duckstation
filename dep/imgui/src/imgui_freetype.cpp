...
bool ImGui_ImplFreeType_FontSrcData::InitFont(FT_Library ft_library, ImFontConfig* src, ImGuiFreeTypeLoaderFlags extra_font_loader_flags)
{
    ...
    if (FT_IS_SFNT(FtFace) && ((error = FT_Get_MM_Var(FtFace, &mmvar)) == 0))
    {
      ...
      {
        if (mmvar->axis[i].tag == FT_MAKE_TAG('w', 'g', 'h', 't'))
          WeightCoordIndex = static_cast<FT_Int>(i);

        VarDesignCoords[i] = mmvar->axis[i].def;
      }

      // Ignore if there was no weight field.
      if (WeightCoordIndex < 0)
      {
          delete[] VarDesignCoords;
...
bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked, void* loader_data_for_baked_src)
{
    ...
    {
        bd_baked_data->FtWeight = static_cast<FT_Fixed>(baked->Weight * static_cast<float>(1 << 16));
